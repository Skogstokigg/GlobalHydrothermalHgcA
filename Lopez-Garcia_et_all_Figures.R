...
all_Sequences_cys <- Sequences_752_strict[1:752,]
all_Sequences_cys <- all_Sequences_cys %>%
  group_by(Site_28) %>%
  mutate(Normalised_Abundance_percent = (Normalised_Abundance_hgcA / sum(Normalised_Abundance_hgcA)) * 100) %>%
  ungroup()
all_Sequences_cys$Site_28 <- factor(all_Sequences_cys$Site_28, levels = c("VFR_Lau Basin", "Hawaii",  "JdF", "Guaymas", "Pescadero", "9º50'N", 
                                                                "Cayman", "Snake Pit", "Rainbow", "SMAR_2", "GAKR_2", "SWIR_3", 
                                                                "SOKI", "OKI", "NWPO", "Manus", "Prony", "Brother Volcano"))

# Prepare data
stack_data <- all_Sequences_cys %>%
...
# Plot
Sequences_bar_plot <- ggplot(all_Sequences_cys) + 
